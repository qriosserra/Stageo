...
<!--                        --><?php //=button("Supprimer", "", Action::ENTREPRISE_DELETE_OFFRE->value . '&id=' . $offre->getIdOffre(),"")?>
                    <?php endif?>
<!--                    --><?php //if(UserConnection::isSignedIn() and UserConnection::isInstance(new Etudiant) and !(new PostulerRepository())->a_Postuler(UserConnection::getSignedInUser()->getLogin(),$offre->getIdOffre())):?>
                    <?php if(UserConnection::isSignedIn() and UserConnection::isInstance(new Etudiant)):?>
